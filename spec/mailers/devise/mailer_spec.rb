...
describe Devise::Mailer do
  ...
  describe "confirmation_instructions" do
    it "should render successfully" do
      lambda { Devise::Mailer.confirmation_instructions(@user) }.should_not raise_error
    end
    describe "rendered without error" do
      before(:each) do
        @mailer = Devise::Mailer.confirmation_instructions(@user)
      end
      it "should set correct subject" do
        @mailer.subject.should == "Instruções de confirmação"
      end
      ...
      it "should set correct body for text html" do
        ...
        filename = Rails.root.join('spec','fixtures','mailers','confirmation_instructions.html')
      ...
      it "should deliver successfully" do
        lambda { Devise::Mailer.confirmation_instructions(@user).deliver }.should_not raise_error
      end
      describe "and delivered" do
        it "should be added to the delivery queue" do
          lambda { Devise::Mailer.confirmation_instructions(@user).deliver }.should change(ActionMailer::Base.deliveries,:size).by(1)
        end
      end
    end
  end
